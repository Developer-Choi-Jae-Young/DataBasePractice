...
/*
    * 계정 생성 * 
    CREATE SUER 계정명 IDENTIFIED BY 비밀번호;
    ...
    * 객체 접근 권한 종류 * 
    ...
    종류      | 접근 객체
    SELECT    | TABELE, VIEW, SEQUENCE      -- 조회(R)
    ...
    --> TEST 계정에 KH계정의 EMPLOYEE 테이블ㅇ르 조회할 수 있는 권한을 부여
...
/*
    ...
    * 트랜젝션 : 데이터베이스의 논리적 연산 단위
                데이터의 변경사항(DML 사용 시)들을 하나의 묶음처럼 트랜젝션에 모아둠
                
    * 트랜젝션 종류
        - COMMIT : 트랜잭션에 담겨져 있는 변경사항들을 실제 DB에 적용하겠다를 의미!
        - ROLLBACK : 트랜잭션에 담겨있는 병경사항들을 삭제(취소) 하고,
                        마지막 COMMIT 위치(시점)로 돌아가는 것을 의미!
        - SAVEPOINT : 현재 시점에 변경사항들을 임시로 저장해두는 것을 의미!
                        ROLLBACK 시 전체 변경사항을 모두 삭제하지 않고 해당 위치까지만 취소가 가능
*/
-- EMP01 테이블 삭제
...
--SAVEPOINT SP시점으로 롤백
ROLLBACK TO SP;     
COMMIT;             --SP 시점으로 돌아간 뒤에 COMMIT을 했기때문에 DELETE 208, 209, 210만 DB에 적용
--------------------------------------------------------------------------------
SELECT * FROM EMP01;
...
ROLLBACK;
-->DDL문(CRETAE, ALTER, DROP)을 사용하게 되면 기존 트랜잭션에 저장된 변경사항들이 무조건 반영됨(COMMIT)
-- DDL문 사용전 DML로 사용된 쿼리들이 있다면 확실하게 처리(COMMIT/ROLLBACK)한 후에 DDL을 사용해야함!!
