...
/*
    * 서브쿼리의 종류 *
      서브쿼리를 수행한 결과값이 몇행 몇열로 나오냐에따라 분류
...
WHERE JOB_CODE = 'J1' AND SALARY = 8000000
    ...
    OR JOB_OCDE = 'J3' AND SALARY = 3400000;
    -- ...;
...
-- => TOP-N분석 : 상위 N개를 조회
-- * ROWNUM : 조회된 순서대로 1부터 순번을 부여해주는 컬럼
...
SELECT EMP_NAME, SALARY, DENSE_RANK() OVER(ORDER BY SALARY DESC) "순위"
FROM EMPLOYEE;
-- WHERE 순위 <= 5; --> 오류 발생! WHERE절에서는 별칭 사용 불가!
...
-- 문제점 (원인) : 부서별 평균급여를 확인해야하는데, 사원 개개인의 급여를 확인했다.
-- 해결방안
SELECT DEPT_CODE, SUM(SALARY) "총합", FLOOR(AVG(SALARY)) AS "평균", COUNT(*) "인원수"
FROM EMPLOYEE
-- WHERE SALARY > 2700000
